...
- Preserved the Tauri release workflow and in-app titlebar update icon behavior, pointed at `TobiasKoehlerETH/APF-SEB-Demo`.
...
- Runtime stream: selected serial port at `115200` baud; APF text lines are parsed as force millinewtons, stale flag, raw counts, temperature mdegC, and timestamp ms.
...
- Keep the titlebar updater icon and GitHub release workflow behavior in sync with this project.
- Do not introduce unrelated binary frames, signal-processing charts, or 3D model assets unless explicitly requested.
